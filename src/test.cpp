...
void fast() {
    Serial.print("hello cruel fast world");
    isFast = 1;
  }

void slow() {
    Serial.print("hello cruel slow world");
    isFast = 0;
  }

void setup_routing() {  	 	 
  server.on("/fast", fast); // calls the wuermli function if you access the page 192.168.1.22/wuermli
  server.on("/slow", slow);
